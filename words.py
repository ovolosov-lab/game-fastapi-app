...
def read_file_sync(path) -> list[list[str]]:
    lines: list[list[str]] = []
...
async def fill_words_list(fileName: str, lang: str, embedder: TextEmbedding, session: SessionDep):
    ...
    try:
        row_words: list[list[str]] = await run_in_threadpool(read_file_sync, file_path)
    except OSError as e:
    ...
    for w, c in row_words:
        if w not in seen and w.strip():
            seen.add(w)
            unique_words.append(w)
            unique_categories.append(int(c))
    ...
    for i in range(0, total_words, BATCH_SIZE):
        ...
        categories = unique_categories[i:i + BATCH_SIZE]
        ...
        await insert_words2db(batch_words, batch_embeddings, categories, lang, session)
...
async def unload_words_list(fileName: str, lang: str, session: SessionDep):
    ...
    sql = text("SELECT w.word, w.cat_id FROM words w WHERE w.language = :lang ORDER BY w.word")
    result = await session.execute(sql, {"lang": lang})

    file_content = "\n".join([f'{row["word"]},{row["cat_id"]}' for row in result.mappings()])
